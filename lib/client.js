var http         = require('http')
  , https        = require('https')
  , url          = require('url')
  , Marshalling  = require('xmlrpc-marshalling')
...
Client.prototype.methodCall = function methodCall(method, params, callback) {
  ...
  var xml       = Marshaller.marshalMethodCall(method, params, options.encoding)
  ...
  var request = transport.request(options, function(response) {
    ...
    if (response.statusCode == 404) {
      ...
      var unmarshaller = new Unmarshaller(options.responseEncoding)

      unmarshaller.unmarshalMethodResponse(response, function(err, result) {
        if (err) {
